...
const key_words_sys_command_need_brackets = ["如果真", "如果", "判断开始", "计次循环首", "判断循环首", "变量循环首"];				// #0000FF 命令可能带括号
const key_words_sys_command = ["程序集变量", "局部变量", "参数"];																 // #0000FF
const key_words_sys_command_ignore_need_brackets = ["如果真结束", "如果结束", "判断结束", "计次循环尾", "判断循环尾", "变量循环尾"]; // #808080 命令可能带括号
const key_words_sys_flowline = ["程序集", "子程序"];																			// #000080
const key_words_sys_flowline_ignore = ["版本", "支持库"];																		// #808080
...
// 处理数字
// @Bug: function("2,3,3",2) -> function("_2_,3,3",_2_) 应该只标记 function("2,3,3",_2_)
function processNumber(temp) {
	// 分割处理非字符串区域
	const parts = temp.split(/(<span[^>]*>.*?<\/span>)/g);
	return parts.map((part, index) => {
		if (index % 2 === 1) return part;
		return part.replace(/(?<![a-zA-Z_\u4e00-\u9fff#%"'“‘’])\b\d+\b(?![a-zA-Z_\u4e00-\u9fff&"'“‘’])/g, match =>
			wrapColor(match, color_code_number)
		);
	}).join('');
}
...
function processFunction(temp) {
	return temp.replace(/([\u4e00-\u9fa5a-zA-Z_][\u4e00-\u9fa5a-zA-Z0-9_]*)\s*[（(]/g, (match, p1) => {
		if (/^\d/.test(p1) || key_words_sys_command_need_brackets.concat(key_words_sys_command_ignore_need_brackets)
			.includes(p1)) return match;
		const bracketPair = match.includes('（') ? '）' : ')';
		if (match.includes(bracketPair)) return match;
		return wrapColor(p1, color_code_command) + match.slice(p1.length);
	});
}

// 处理常量
function processConstant(temp) {
	return temp.replace(/(?<![a-zA-Z_\u4e00-\u9fff#"'“‘’])#[\u4e00-\u9fa5a-zA-Z_][\u4e00-\u9fa5a-zA-Z0-9_]*/g, match =>
		wrapColor(match, color_constant_y)
	);
}
...
function processLine(line) {
	...
	let result = line;
	result = processStrings(result); 	// 处理字符串
		...
		if (codeBlockMatch) {
			...
			for (let j = 0; j < lines.length; j++) {
				...
				if (commentIndex !== -1) {
					...
					highlightedLine = processLine(codePart) + wrapColor(commentPart, color_remark);
				} else {
					highlightedLine = processLine(line);
				}
				...
				if (line.trim().startsWith(".")) {
					...
					if (key_words_sys_command.concat(key_words_sys_command_need_brackets).includes(command)) {
						highlightedLine = lineTrim + wrapColor(parts[0], color_sys_command) + highlightedLine.substring(spaceCount + parts[0].length);
					} else if (key_words_sys_command_ignore_need_brackets.includes(command)) {
						highlightedLine = lineTrim + wrapColor(parts[0], color_sys_command_ignore) + highlightedLine.substring(spaceCount + parts[0].length);
					} else if (key_words_sys_flowline.includes(command)) {
						highlightedLine = lineTrim + wrapColor(parts[0], color_function) + highlightedLine.substring(spaceCount + parts[0].length);
					} else if (key_words_sys_flowline_ignore.includes(command)) {
...
function switchToEdit(pos) {
	...
	outputTextarea.scrollTop = temp * (outputTextarea.scrollHeight - outputTextarea.clientHeight);
	setCursorPosition(outputTextarea, cursorPosition)
}
...
function switchToHighlight() {
	...
	highlightOutput();
	outputDiv.scrollTop = temp * (outputDiv.scrollHeight - outputDiv.clientHeight);
}