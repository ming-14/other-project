...
def _get_crash_log_path():
    return os.path.join(_logs_dir, f'crash_{_now_file_ts()}.log')
...
def _record_crash(title, detail, log_path=None):
    global _crash_info
    with _crash_lock:
        _crash_info = (title, detail, log_path)


def _show_native_dialog(title, detail, log_path=None):
...
def _notify_crash(title, detail, log_path=None):
    if _notify_handler is not None:
        try:
            _notify_handler(title, detail)
            return
        except BaseException:
            pass
    _show_native_dialog(title, detail, log_path)


def set_notify_handler(handler):
    """Set custom crash notification handler.

    Args:
        handler: callable(title: str, detail: str) -> None
                 Called on crash. If it raises, falls back to native dialog.
                 Pass None to restore default (native dialog).
    """
...
def _setup_windows_uef():
    ...
    def uef_filter(exception_pointers):
        try:
            ...
            log_path = _write_crash_log(f'Unhandled SEH exception: {name}', detail)
            _record_crash(f'Unhandled SEH exception: {name}', detail, log_path)
...
_DEFAULT_IDLE_PATTERNS = [
    'WaitForSingleObject', 'WaitForMultipleObjects',
    'NtWaitForSingleObject', 'MsgWaitForMultipleObjects',
    'select', 'poll', 'epoll_wait', 'kevent',
]

_idle_patterns = list(_DEFAULT_IDLE_PATTERNS)


def add_idle_pattern(pattern):
    """Add a string pattern to recognize idle (non-stuck) main thread stacks.

    If the main thread's stack contains this string, the watchdog
    will not report a hang. Useful for GUI framework event loops.

    Args:
        pattern: Substring to match in the formatted stack trace.
    """
    if pattern not in _idle_patterns:
        _idle_patterns.append(pattern)
...
def install(app_name=None, watchdog_timeout=30):
    ...
    def crash_handler(exc_type, exc_value, exc_tb):
        try:
            ...
            log_path = _write_crash_log(title, full_content)
            _record_crash(title, tb_text, log_path)
        except Exception:
            pass
        _original_excepthook(exc_type, exc_value, exc_tb)
    ...
    if hasattr(threading, 'excepthook'):
        ...
        def threading_exception_hook(args):
            try:
                ...
                log_path = _write_crash_log(title, full_content)
                _record_crash(title, f"Thread {args.thread.name}: {tb_text}", log_path)
            except Exception:
                pass
    ...
    try:
        def sig_handler(sig, frame):
            try:
                ...
                log_path = _write_crash_log(title, full_content)
                _record_crash(title, stack, log_path)
            except Exception:
                pass
...
def show_last_crash():
    ...
    with _crash_lock:
        if _crash_info is None:
            ...
            return
        title, detail, log_path = _crash_info
    _notify_crash(title, detail, log_path)
